...
void ChcInterpreterContext::interpretCheckSat() {
    ...
    if (engineName.find(',') != std::string::npos) {
        ...
        for (auto const & name : std::views::split(engineName, ',')) {
            engines.emplace_back(name.begin(), name.end());
        }
        pid_t parent = getpid();
...
std::optional<ChClause> ChcInterpreterContext::chclauseFromPTRef(PTRef ref) {
    ...
    if (matchingSubTerms(logic, interpretedPart, [&](PTRef term) { return this->isUninterpretedPredicate(term); }).size() > 0) {
        return std::nullopt;
    }
