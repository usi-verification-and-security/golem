...
namespace golem {
bool TransitionSystem::isWellFormed() const {
...
PTRef TransitionSystem::toNextStateVar(PTRef var) const {
    ...
    std::string const newName = std::move(originalName) + suffix;
...
SystemType::SystemType(std::vector<SRef> stateVarTypes, std::vector<SRef> auxiliaryVarTypes, Logic & logic)
    ...
    std::ranges::transform(stateVarTypes, std::back_inserter(stateVars),
                           [&](SRef sref) { return tm.getVarVersionZero(helper(sref)); });
    std::ranges::transform(stateVars, std::back_inserter(nextStateVars),
                           [&](PTRef var) { return tm.sendVarThroughTime(var, 1); });
    helper.varNamePrefix = "aux";
    std::ranges::transform(auxiliaryVarTypes, std::back_inserter(auxiliaryVars),
                           [&](SRef sref) { return tm.getVarVersionZero(helper(sref)); });
}
...
TransitionSystem TransitionSystem::reverse(TransitionSystem const & original) {
    PTRef const reversedInitial = original.query;
    PTRef const reversedQuery = original.init;
    PTRef const reversedTransition = reverseTransitionRelation(original);
...
PTRef TransitionSystem::reverseTransitionRelation(TransitionSystem const & transitionSystem) {
    ...
    TimeMachine const tm(transitionSystem.logic);
    ...
    std::ranges::transform(stateVars, std::back_inserter(helperVars),
                           [&](PTRef var) { return tm.sendVarThroughTime(var, 2); });
    TermUtils const utils(transitionSystem.logic);
    TermUtils::substitutions_map subst;
    std::size_t const varCount = stateVars.size();
...
PTRef KTo1Inductive::qe(PTRef invariant, unsigned k, TransitionSystem const & system) {
    ...
    PTRef const transition = system.getTransition();
    std::vector<PTRef> const stateVars = system.getStateVars();
    ...
    PTRef const afterElimination = QuantifierElimination(logic).keepOnly(acc, stateVars);
