...
private:
    ...
    std::tuple<Answer, PTRef> tryTransitionInvariant(PTRef init, PTRef transition, PTRef sink, PTRef trace, uint num, ArithLogic & logic, vec<PTRef> & strictCandidates);

    std::tuple<Answer, PTRef> refineTransitionInvariant(PTRef init, PTRef transition, PTRef sink, PTRef trInv, ArithLogic & logic, vec<PTRef> & strictCandidates);
};
} // namespace golem::termination
