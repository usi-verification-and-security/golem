...
struct Expr {
    enum struct Kind { And, Or, Eq, Add, Var, Const } kind;
...
LocationDeclaration ITS::parseFun(SExpression const & expr) {
	...
	if (args.size() != 3 or asAtom(args[0]) != "fun")
		throw std::runtime_error("Invalid (fun ...) form");

	auto const & name = asAtom(args[1]);
	auto const & arrowExpr = asSubExpressions(args[2]);

	if (asAtom(arrowExpr[0]) != "->")
		throw std::runtime_error("Function type must use (-> ...)");
...
Location ITS::parseLocationInstance(SExpression const & expr) {
	...
	if (args.empty())
		throw std::runtime_error("Location instance cannot be empty");
...
Rule ITS::parseRule(SExpression const & expr) {
	auto const & args = asSubExpressions(expr);
	if (args.empty() or asAtom(args[0]) != "rule")
	...
	if (asAtom(args[3]) != ":guard")
		throw std::runtime_error("Expected :guard keyword");
...
std::shared_ptr<Expr> ITS::parseExpr(SExpression const & expr) {
	...
	Expr::Kind kind;
	if (op == "and") kind = Expr::Kind::And;
	else if (op == "or") kind = Expr::Kind::Or;
	else if (op == "=") kind = Expr::Kind::Eq;
	else if (op == "+") kind = Expr::Kind::Add;
...
ChcSystem ITS::asChcs(ArithLogic & logic) const {
	...
	for (auto && [_, locationDeclaration] : this->locations) {
		assert(std::all_of(locationDeclaration.argTypes.begin(), locationDeclaration.argTypes.end(), [](auto const & typeName) {
			return typeName == "Int";
		}));
		auto const predicate = logic.declareFun(locationDeclaration.name, logic.getSort_bool(), std::vector(locationDeclaration.argTypes.size(), logic.getSort_int()));
...

	{
		...
		chcs.addClause(ChcHead{.predicate = {entryPredicate}}, ChcBody{.interpretedPart = {logic.getTerm_true()}, .uninterpretedPart = {}});
	}
	return chcs;
}
...
void run(std::string const & filename, Options const & options) {
	...
	if (res == LassoDetector::Answer::LASSO) {
		std::cout << "Non-terminating!" << std::endl;
	} else if (res == LassoDetector::Answer::NO_LASSO) {
		std::cout << "No lasso => Maybe terminating" << std::endl;
	} else {
