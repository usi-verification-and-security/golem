...
LassoDetector::Answer LassoDetector::find_lasso(TransitionSystem const & system) {
    ...
    std::size_t varCount = system.getStateVars().size();
    // Init clause
    auto notYetSaved = [&]() -> PTRef {
        ...
        for (std::size_t i = 0u; i < varCount; ++i) {
            args.push(zero);
        }
        args.push(logic.getTerm_false());
        return logic.mkUninterpFun(predicate, std::move(args));
    }();
    chcs.addClause(ChcHead{UninterpretedPredicate{notYetSaved}},
    ...
    // no saving
    {
        ...
        chcs.addClause(ChcHead{UninterpretedPredicate{head}}, ChcBody{InterpretedFla{system.getTransition()}, {UninterpretedPredicate{body}}});
    }
    // saving
    {
        PTRef head = [&]() -> PTRef {
        ...
        chcs.addClause(ChcHead{head}, ChcBody{InterpretedFla{system.getTransition()}, {UninterpretedPredicate{notYetSaved}}});
    }
    // query
    {
        PTRef body = [&]() -> PTRef {
        ...
        chcs.addClause(ChcHead{logic.getTerm_false()}, ChcBody{InterpretedFla{logic.getTerm_true()}, {UninterpretedPredicate{body}}});
    }
