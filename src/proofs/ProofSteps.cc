...
std::vector<std::shared_ptr<Term>> literals(std::shared_ptr<Term> const & term1,
...
std::vector<std::shared_ptr<Term>> literals(std::shared_ptr<Term> const & term1, std::shared_ptr<Term> const & term2,
    ...
    return std::vector{term1, term2, term3};
}

std::vector<std::shared_ptr<Term>> args(std::shared_ptr<Term> const & term) {
    return std::vector{term};
}

std::vector<std::shared_ptr<Term>> args(std::shared_ptr<Term> const & term1,
                                              std::shared_ptr<Term> const & term2) {
    return std::vector{term1, term2};
}
...
void StepHandler::recordStep(std::vector<std::shared_ptr<Term>> && clause, std::string rule, Step::Premises && premises) {
    notifyObservers(Step{currentStep++, Step::StepType::STEP, std::move(clause), std::move(rule), std::move(premises)});
}

void StepHandler::recordForallInstStep(std::vector<std::shared_ptr<Term>> && clause, InstantiationPairs && instantiationPairs) {
    notifyObservers(Step{currentStep++, Step::StepType::STEP, std::move(clause), "forall_inst", std::move(instantiationPairs)});
}
...
void StepHandler::buildIntermediateProof() {
    ...
    for (std::size_t i = 0; i < derivationSize; ++i) {
        ...
        for (std::size_t premise : step.premises) {
            if (premise == 0) { continue; } // Premise 0 is the helper fact "true", we do not have it in the Alethe proof
...
void StepHandler::buildAletheProof() {
    ...
    for (std::size_t i = 0; i < derivationSize; ++i) {
        ...
        if (step.premises.empty()) { assert(step.derivedFact == logic.getTerm_true()); continue; }
        ...
        recordStep(literals(negate(std::make_shared<Op>("!", literals(implicationLHS,
            makeName(":named @LHS" + std::to_string(i - 1))))),
            implicationRHS), "implies", Step::Premises{lastStep()});
...
StepHandler::SimplifyResult StepHandler::simplify(TermPtr const & term, std::optional<TermPtr> name) {
    ...
    if (type == Term::LET or type == Term::QUANT) { throw std::logic_error("Let and quantified term should not occur here anymore!"); }
    ...
    auto processResults = [this](TermPtr const & term, std::optional<TermPtr> & name, std::vector<SimplifyResult> const & results) {
        ...
        auto namedTerm = name.has_value() ? name.value() : std::make_shared<Op>("!", std::vector<TermPtr>{term, makeName(":named @T" + std::to_string(nameIndex))});
    ...
    if (type == Term::OP) {
        ...
        if (op->getOp() == "ite") {
            return shortCircuitSimplifyITE(op);
        }
        auto const & args = op->getArgs();
        auto results = simplifyArgs(args);
        bool changed = std::any_of(results.begin(), results.end(), [](auto const & result) { return result.has_value(); });
        ...
        if (changed) { // Derive that the original op is equiv to evaluatedTerm by transitivity
            ...
            recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{name.value(), evaluatedTerm})), "trans", {firstEquivStep, lastStep()});
        }
...
StepHandler::SimplifyResult StepHandler::shortCircuitSimplifyITE(std::shared_ptr<Op> const & ite) {
    ...
    if (res) {
        ...
        recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{ite, iteSimplified})), "cong", Step::Premises{res->second});
        finalPremises.premises.push_back(lastStep());
    }
    assert(conditionSimplified->getTermType() == Term::TERMINAL and conditionSimplified->getTerminalType() == Term::BOOL);
    ...
    if (branchSimplificationResult or res) {
        ...
        recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{ite, simplified})), "trans", std::move(finalPremises));
    }
...
StepHandler::TermPtr StepHandler::simplifyOpDirect(std::shared_ptr<Op> const & opTerm) {
    ...
    if (op == "=") {
        ...
        recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{negated, innerSimplifiedNegated})), "cong", {lastStep()});
        auto secondEq = lastStep();
        simplified = simplifyOpDirect(innerSimplifiedNegated);
        recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{opTerm, simplified})), "trans", {firstEq, secondEq, lastStep()});
        return simplified;
    } else if (op == ">=") {
        assert(args.size() == 2);
        auto swapped = std::make_shared<Op>("<=", std::vector<std::shared_ptr<Term>>{args[1], args[0]});
        ...
        recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{opTerm, simplified})), "trans", Step::Premises{firstEq, lastStep()});
        return simplified;
    } else if (op == "and") {
        ...
        for (auto const & arg : args) {
            if (arg->getTermType() == Term::TERMINAL and arg->getTerminalType() != Term::VAR) {
                ...
                if (val == "false") {
                    recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{opTerm, arg})), "and_simplify", {});
                    return arg;
                } else {
                    assert(val == "true");
                }
        ...
        bool isTrue = std::any_of(args.begin(), args.end(), [](auto const & arg) {
            return arg->getTerminalType() == Term::BOOL and std::dynamic_pointer_cast<Terminal>(arg)->getVal() == "true";
        });
        if (not isTrue) {
            assert(std::all_of(args.begin(), args.end(), [](auto const & arg) {
                return arg->getTerminalType() == Term::BOOL and std::dynamic_pointer_cast<Terminal>(arg)->getVal() == "false";
            }));
        }
        simplified = std::make_shared<Terminal>(boolToString(isTrue), Term::BOOL);
        rule = "or_simplify";
...
public:
    ...
    std::pair<std::shared_ptr<Term>, bool> removeUnusedQuantifiers(std::shared_ptr<Term> const & term) {
        ...
        return {std::make_shared<Quant>(quantifiedTerm->getQuant(), std::move(newVars), std::move(newSorts), quantifiedTerm->getCoreTerm()), true};
    }
...
StepHandler::TermPtr StepHandler::instantiationSteps(std::size_t i, TermPtr quantifiedTerm) {
    ...
    recordForallInstStep(literals(std::make_shared<Op>(
        "or",
        literals(negate(namedAssumption),
            std::make_shared<Op>("!", literals(instantiatedTerm, makeName(":named " + instantiationReNamedTerm->printTerm())))))),
        std::move(instPairs));
...
void StepHandler::buildAssumptionSteps() {
    for (auto & assertion : originalAssertions) {
        ...
        recordAssumption(literals(std::make_shared<Op>(
            "!", args(assertion, makeName(":named @a" + std::to_string(currentStep)))
        )));
    }
}
