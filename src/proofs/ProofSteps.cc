...
std::string Step::printStepAlethe() const {
    ...
    auto const& presimeNumbers = premises.premises;
    if (not presimeNumbers.empty()) {
        ss << " :premises (";
        for (std::size_t i = 0; i < presimeNumbers.size(); i++) {
            ss << "t" << presimeNumbers[i];
            if (i != presimeNumbers.size() - 1) { ss << " "; }
        }
        ss << ")";
    }
...
void StepHandler::buildIntermediateProof() {
    ...
    //         recordForallInstStep(literals(currTerm), std::move(instPairs));
    //         currTerm = currTerm->accept(&instantiateVisitor);
    //     }
    //
    //     notifyObservers(Step(currentStep, Step::StepType::STEP, literals(currTerm)));
    //     currentStep++;
    //
...
void StepHandler::buildAletheProof() {
    ...
    for (std::size_t i = 0; i < derivationSize; ++i) {
        ...
        recordStep(literals(negate(std::make_shared<Op>("!", literals(implicationLHS,
            makeName(":named @impLHS" + std::to_string(i - 1))))),
            implicationRHS), "implies", Step::Premises{lastStep()});
        auto implicationStep = lastStep();

        std::shared_ptr<Term> renamedImpLHS = makeName("@impLHS" + std::to_string(i - 1));
        ...
        auto simplificationResult = simplify(implicationLHS);
...
StepHandler::SimplifyResult StepHandler::simplify(TermPtr const & term) {
    ...
    // Create simplified
    auto processResults = [this](TermPtr const & term, std::vector<SimplifyResult> const & results) {
        ...
        auto congruenceTerm = std::make_shared<Op>("=", std::vector<TermPtr>{term, simplifiedTerm});
    ...
    if (type == Term::APP) { // Uninterpreted predicate
        ...
        auto simplifiedTerm = processResults(term, results);
    ...
    if (type == Term::OP) {
        ...
        if (changed) {
            opWithArgsSimplified = processResults(term, results);
            firstEquivStep = lastStep();
        }
        auto evaluatedTerm = std::dynamic_pointer_cast<Op>(opWithArgsSimplified)->operate();
        recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{opWithArgsSimplified, evaluatedTerm})), op->simplifyRule(), {});
        if (changed) { // Derive that the original op is equiv to evaluatedTerm by transitivity
            assert(firstEquivStep != -1);
            recordStep(literals(std::make_shared<Op>("=", std::vector<TermPtr>{op, evaluatedTerm})), "trans", {firstEquivStep, lastStep()});
        }
