...
namespace {
    ...
    CommandArgs dropOne(std::vector<SExpression> const & args) {
        return CommandArgs(args.begin() + 1, args.end());
    }
...
public:
    VMTModel(
        std::shared_ptr<opensmt::Logic> logic,
        ...
        ) : logic(logic), stateVars(std::move(stateVars)), nextStateVars(std::move(nextStateVars)),
            auxVars(std::move(auxVars)), init(init), transition(transition), invarProperties(std::move(invarProperties))
    {}

    ChcSystem asChcs() const;
...
private:
    ...
    void resolveAssert(SExpression const & arg);
...
void VMTModelBuilder::resolveAssert(SExpression const & arg) {
    // Ignore for now
}
...
VMTModel VMTModelBuilder::build() {
    ...
    for (auto const & [name, type, def] : defs) {
        ...
        PTRef term = [&]() -> PTRef {
            if (op == "!") {
                ...
                auto const & annotation = asAtom(subExpressions[2]);
                if (annotation == ":init") {
                    gatherer.init = term;
                } else if (annotation == ":trans") {
                    gatherer.trans = term;
                } else if (annotation == ":invar-property") {
                    unsigned index = std::stoull(asAtom(subExpressions[3]));
                    gatherer.invarProperties.insert({index, term});
                } else if (annotation == ":next") {
                    auto const & nextVarName = asAtom(subExpressions[3]);
    ...
    std::vector<PTRef> auxVars;
    // TODO: Fill aux vars
    return VMTModel(std::move(logic), std::move(stateVars), std::move(nextStateVars), auxVars, gatherer.init,
