...
ModelBasedProjection::implicant_t ModelBasedProjection::projectSingleVar(PTRef var, implicant_t implicant,
                                                                         Model & model) {
    ...
    if (logic.hasSortBool(var)) {
        ...
        if (it != implicant.end()) {
            ...
            assert(std::find_if(it, implicant.end(), [var](PtAsgn literal) { return literal.tr == var; }) ==
                   implicant.end());
        }
        return implicant;
    }
    ...
    auto containsVar = [var, &utils](PtAsgn lit) { return utils.atomContainsVar(lit.tr, var); };
        ...
        if (logic.isEquality(lit.tr)) {
            ...
            if (lit.sgn == l_True) { // if equality is present, we just use it to substitute the variable away
                ...
                for (std::size_t i = 0; i < size; /* manual control */) {
                    ...
                    if ((literal.tr == logic.getTerm_true() and literal.sgn == l_True) or
                        (literal.tr == logic.getTerm_false() and literal.sgn == l_False)) {
                ...
                if (replacement == logic.getTerm_true()) {
                    ...
                    // strict inequalities are expressed as negations of non-strict ones
                    assert(logic.isNot(replacement));
                    // we store the non-strict inequality, the sign is already set to false
                    it->tr = logic.getPterm(replacement)[0];
    ...
    for (auto it = implicant.begin(); it != interestingEnd; ++it) {
        ...
        if (ineq == logic.getTerm_true()) {
            // this is still possible if we had true disequality at the beginning (x != x + 1)
            assert(sign == l_True);
            continue;
        }
        ...
        auto interestingVarIt =
            std::find_if(factors.begin(), factors.end(), [var](LinearFactor factor) { return factor.var == var; });
        ...
        if (coeff.sign() < 0) { isLower = !isLower; }
        ...
        for (auto & factor : factors) { // in place update
            ...
            boundArgs.push(lalogic->mkTimes(factor.var, factor.coeff));
        }
...
ModelBasedProjection::VarsInfo computeVarsInfo(PTRef fla, Logic & logic, PTRef const * const beg,
                                               PTRef const * const end) {
...
void checkImplicant(ModelBasedProjection::implicant_t const & implicant, Logic & logic, Model & model) {
    ...
    for (auto const & elem : implicant) {
        ...
        assert((elem.sgn == l_False and model.evaluate(elem.tr) == logic.getTerm_false()) or
               (elem.sgn == l_True and model.evaluate(elem.tr) == logic.getTerm_true()));
    }
}
} // namespace
...
PTRef ModelBasedProjection::project(PTRef fla, const vec<PTRef> & varsToEliminate, Model & model) {
    ...
    auto separator = std::stable_partition(implicant.begin(), implicant.end(), [&varsInfo](PtAsgn lit) {
        bool hasVar = true;
        if (varsInfo.peek(lit.tr, hasVar)) { return hasVar; }
...
ModelBasedProjection::implicant_t ModelBasedProjection::projectIntegerVars(PTRef * beg, PTRef * end,
    ...
    auto divConstraintsBeg = std::stable_partition(implicant.begin(), implicant.end(),
                                                   [&](PtAsgn lit) { return not isDivisibilityConstraint(lit); });
...
template<class ForwardIt, class Funct> ForwardIt maxElementWithProjection(ForwardIt first, ForwardIt last, Funct f) {
 ...
 */
void ModelBasedProjection::processDivConstraints(PTRef var, div_constraints_t & divConstraints, implicant_t & implicant,
                                                 Model & model) {
    auto & lialogic = dynamic_cast<ArithLogic &>(logic);
    assert(lialogic.hasIntegers());
    auto itInterestingEnd =
        std::partition(divConstraints.begin(), divConstraints.end(), [&](DivisibilityConstraint const & c) {
    ...
    if (itInterestingEnd != divConstraints.begin()) {
        ...
        // v -> u + d * v_tmp
        subst.insert(
            {var, lialogic.mkPlus(lialogic.mkIntConst(u), lialogic.mkTimes(lialogic.mkIntConst(d), replacementVar))});
 ...
 */
void ModelBasedProjection::processClassicLiterals(PTRef var, div_constraints_t & divConstraints,
                                                  implicant_t & implicant, Model & model) {
    auto & lialogic = dynamic_cast<ArithLogic &>(logic);
    assert(lialogic.hasIntegers());
    ...
    auto containsVar = [var, &lialogic](PtAsgn lit) { return LATermUtils(lialogic).atomContainsVar(lit.tr, var); };
    // split the literals to those containing var and those not containing var
    auto interestingEnd = std::partition(implicant.begin(), implicant.end(), containsVar);
    ...
    if (equal.empty()) {
        ...
        for (auto it = equal.begin() + 1; it != equal.end(); ++it) {
            ...
            assert(lialogic.getNumConst(otherBound.coeff).sign() > 0 and
                   lialogic.getNumConst(otherBound.coeff).isInteger());
            ...
            if (newLit != lialogic.getTerm_true()) { newLiterals.emplace_back(newLit, l_True); }
        }
        // lower bounds
        for (auto const & lowerBound : lower) {
            assert(lialogic.getNumConst(lowerBound.coeff).sign() > 0 and
                   lialogic.getNumConst(lowerBound.coeff).isInteger());
            // ax = t; s <= bx => as <= bt
        ...
        for (auto const & upperBound : upper) {
            assert(lialogic.getNumConst(upperBound.coeff).sign() > 0 and
                   lialogic.getNumConst(upperBound.coeff).isInteger());
            // ax = t; s >= bx => as >= bt
...
ModelBasedProjection::ResolveResult ModelBasedProjection::resolve(LIABoundLower const & lower,
    ...
    if (mul <= (b * t - a * s)) {
        ...
        if (nBound != lialogic.getTerm_true()) { result.bounds.push_back(nBound); }
        result.hasDivConstraint = false;
        return result;
    }

    PTRef firstBound = lialogic.mkLeq(as_term, bt_term);
    if (firstBound != lialogic.getTerm_true()) { result.bounds.push_back(firstBound); }
    if (a >= b) {
        // case 2
        ...
        if (not d.isZero()) {
            ...
            if (secondBound != lialogic.getTerm_true()) { result.bounds.push_back(secondBound); }
        }
