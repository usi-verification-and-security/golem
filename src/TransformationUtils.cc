...
std::unique_ptr<TransitionSystem> toTransitionSystem(ChcDirectedGraph const & graph) {
    ...
    graph.forEachEdge([&](DirectedEdge const & edge) {
        ...
        if (isInit) {
            ...
            std::transform(systemVariables.nextStateVars.begin(), systemVariables.nextStateVars.end(), stateVars.begin(),
                           std::inserter(subMap, subMap.end()),
                           [](PTRef key, PTRef value) { return std::make_pair(key, value); });
            std::transform(systemVariables.auxiliaryVars.begin(), systemVariables.auxiliaryVars.end(), auxiliaryVars.begin(),
                           std::inserter(subMap, subMap.end()),
                           [](PTRef key, PTRef value) { return std::make_pair(key, value); });
