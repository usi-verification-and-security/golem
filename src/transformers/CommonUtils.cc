...
PTRef EdgeTranslator::translateEdge(DirectedEdge const & edge) {
    ...
    PTRef updatedLocation = [&]() {
        ...
        for (auto && entry : locationVarMap) {
            if (entry.second != targetLocationVar) {
                args.push(
                    logic.mkNot(timeMachine.sendVarThroughTime(entry.second, source != graph.getEntry() ? 1 : 0)));
            }
            if (entry.second != sourceLocationVar && source != graph.getEntry()) {
    ...
    vec<PTRef> components{sourceLocationVar, translatedConstraint,
                          timeMachine.sendVarThroughTime(targetLocationVar, source != graph.getEntry() ? 1 : 0),
                          updatedLocation};
