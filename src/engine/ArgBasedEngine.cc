...
void Algorithm::computeNewUnprocessedEdges(ARG::NodeId nodeId) {
    ...
    for (EId edge : candidateClauses) {
        ...
        for (auto & nodeInstances : allInstances) {
            nodeInstances.erase(std::remove_if(nodeInstances.begin(), nodeInstances.end(), [this](ARG::NodeId nodeId) {
                return arg.isCovered(nodeId);
            }), nodeInstances.end());
        }
        ...
        for (; indices[0] != allInstances[0].size(); increment(indices, allInstances)) {
            ...
            assert(std::none_of(argSources.begin(), argSources.end(), [this](auto sourceId) { return arg.isCovered(sourceId); }));
