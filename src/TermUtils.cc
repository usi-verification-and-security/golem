    ...
    static Cubes conjunctionAsCubes(std::vector<Cubes> && toConjoin) {
        ...
        auto dfs = [&](auto&& self, std::size_t depth) -> void {
            if (depth == toConjoin.size()) {
                ...
                auto it = std::adjacent_find(literals.begin(), literals.end(), [](PtAsgn const & p1, PtAsgn const & p2) -> bool {
                    return p1.tr == p2.tr && p1.sgn != p2.sgn;
                });
...
void DNFTransformer::run(PTRef fla) {
    ...
    if (logic.isOr(fla)) {
        ...
        for (PTRef const child : logic.getPterm(fla)) { run(child); }
        std::vector<Cubes> childrenCubes;
        childrenCubes.reserve(logic.getPterm(fla).size());
        for (PTRef const child : logic.getPterm(fla)) { childrenCubes.push_back(cubes.at(child)); }
